enum ledStates {INCREASE, DECREASE, STAY, WAVE, OFF, ON, DIM}; // Here we make nicknames for the different states our program supports.
...
void compose() {
  ...
  case WAVE:
    Serial.print("WAVE  ");
    Serial.println(brightness);
    doForMs(10000, wavyshine); // this you might want to do for number of pulses, rather than for duration
    ledState = WAVE;
    break;
...
void wavyshine(){
  Serial.print("WAVE  ");
  Serial.println(brightness);
  brightness = sinewave(100,200,50); // you can tweak the parameters of the sinewave
  analogWrite(ledPin, brightness);
  }
